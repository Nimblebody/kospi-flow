...
# ---------------------------------------------------------------- 증시 해설
# 없으면 '왜 움직였나' 꼭지만 빠지고 리포트는 그대로 나간다.
...
# ---------------------------------------------------------------- 실행 시각
# 이 시각(KST) 전에는 그날 수급이 아직 없는 게 정상이다. 코스피는 15:30 에
# 끝나고 확정 수급은 그 뒤에 올라온다.
#
# GitHub 크론은 제때 발사된다는 보장이 없다. 실제로 09:30 UTC 예약이
# 19:53 UTC 에 돈 적이 있다(KST 04:53). 그 시각엔 당일 데이터가 있을 리 없으므로,
# 데이터가 비었을 때 실패로 볼지 정상 종료로 볼지 이 값으로 가른다.
DATA_READY_KST = (16, 10)  # 시, 분

# 코스피 정규장 마감. 이 시각 전에는 당일 수급이 존재할 수 없으므로
# 752종목을 3분간 긁어보지도 않고 바로 끝낸다.
MARKET_CLOSE_KST = (15, 30)

# 미국 시장 시간대. 서머타임이 있어 고정 오프셋으로 두면 반년마다 한 시간 틀어진다.
US_TZ = ZoneInfo("America/New_York")

# 미 정규장 마감. KIS 해외주식 일봉은 아직 안 끝난 세션도 한 행으로 주기 때문에
# (프리마켓 값이 그날 날짜로 들어온다) 이 시각 전의 당일 행은 버린다.
US_CLOSE = (16, 0)
