...
- [x] 38. src/news.py — 국내 RSS 8곳 + 구글 뉴스 보충, 종목 키워드로 거르기
- [x] 39. 요약·중요 20건 선정 (모델은 번호만 고르고 매핑은 파이썬)
      검증: tests/test_news.py 10개 (범위 밖·중복·비정수 번호 처리 포함)
- [x] 40. 뉴스 탭 + panelNews. 직접링크/구글 경유 구분 표시
      검증: node 로 실제 렌더 실행
- [x] 41. .github/workflows/news.yml
- [x] 42. Worker 가 크론마다 다른 워크플로를 부르도록 (01:30 KST = news.yml)
      검증: 크론 매핑·기준 날짜 node 로 실행 확인
- [x] 43. 실제 AI 호출 확인 완료. 첫 실행은 스키마에 additionalProperties: False
      가 빠져 실패했다. 고친 뒤 요약 품질은 쓸 만했다
- [x] 44. 두 출처의 수집 창이 달라 화면 날짜가 어긋나던 것 수정.
      창은 '어제 00:00 ~ 지금' 을 유지하고 화면에 구간을 그대로 적는다

---

## 10차 · 애프터마켓 대응 (2026-09-16)

- [x] 45. KRX 애프터마켓(09-14 시행) 조사. 정규장 15:30 그대로 → 기존 설정 유효
- [x] 46. 애프터마켓이 수급 정정을 키웠는지 실측. 시행 전후 비교 결과 증거 없음
- [x] 47. 20:30 확정 갱신 크론 추가 (Worker refresh 잡). 20:00 재시도는 대체
- [ ] 48. 한 달 뒤(2026-10 중순) 같은 측정을 다시 해 볼 것.
      애프터마켓 거래가 늘면 결과가 달라질 수 있다

## 11차 · 장중 단타 스크리너 (2026-09-17 시작)

계획. HTS [0110] 에서 짜 둔 조건검색 결과를 장중에 3초마다 받아 보여준다.
매매 기준(거래대금·거래량·체결강도·이평선·차트패턴)은 HTS 조건 안에서 처리되고,
앱은 걸린 종목을 받아 보여주는 데 집중한다.

    PWA 스크리너 탭 (화면이 보일 때만 3초마다)
      -> Worker /screen/titles           조건 목록 (처음 한 번)
      -> Worker /screen/result?seq=N     조건 결과
           -> KIS psearch-title / psearch-result (HHKST03900300 / 400)
           -> Worker 메모리 캐시 2.5초. 기기 여러 대가 불러도 KIS 는 조건당 3초에 1번

- [ ] 49. Worker 에서 KIS(:9443) 에 닿는지 확인 — /kis-ping
      안 닿으면 구조를 다시 짠다 (다른 중계 호스트)
- [ ] 50. HTS [0110] 에 조건 저장 + '사용자조건 서버저장'. KIS_HTS_ID 를 .env 에
- [ ] 51. 로컬에서 psearch-title / psearch-result 실측 (응답 필드·시간 제한 확인)
- [ ] 52. Worker /screen/* — KIS 토큰은 KV 에 보관(발급이 분당 1회로 막혀 있음),
      결과는 메모리 캐시, 키는 트리거 키와 분리(SCREEN_KEY)
- [ ] 53. 스크리너 탭 — 조건 칩, 3초 갱신(화면 숨으면 멈춤), 정렬(체결강도/거래대금),
      새로 걸린 종목 표시. 서버 주소·키는 설정 시트에 입력(공개 페이지에 박지 않음)
- [ ] 53b. 앱에서 문턱 조절 — 체결강도 하한·거래대금 하한·등락률 범위·시총 범위.
      조건검색 결과에 값이 이미 있으므로 브라우저에서만 거른다(추가 호출 0).
      기기별로 기억(localStorage). 거르는 로직은 순수 함수로 두고 node 로 테스트
- [ ] 54. 테스트 · 문서 · 커밋

## 미룬 것

- [ ] 15:40 API 잠금 반영. 확정 수급 API(FHPTJ04160001)가 00:00~15:40 에 호출을
      막는다. too_early_to_collect 기준을 15:30 -> 15:40 으로. 그리고 9/2 노트의
      '15:34 에 데이터가 없었다' 는 해석을 '호출이 막혀 있었다' 로 바로잡을 것
...
01:30 KST 크론이 처음 도는 날이다.

- [ ] Cloudflare Logs 에 `cron 30 16 * * * (뉴스)` 가 찍혔는가
- [ ] 사이트 뉴스 탭에 그날 요약이 떴는가
- [ ] 요약 문장과 중요 기사 20건 선정이 납득이 가는가
      (어색하면 src/news.py 의 _prompt 를 손본다)
- [ ] 중요 기사 링크가 실제로 열리는가. '구글 뉴스 경유' 표시가 붙은 건
      한 번 더 튄다

RSS 피드는 최근 것만 들고 있어서 하루만 지나도 빠진다. 09-01 기사는
직접링크가 34건인데 09-02 는 142건이었다. 지난 뉴스를 소급해서 만드는 건
잘 안 되므로 매일 제때 도는 게 중요하다.
