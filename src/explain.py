...
4. conflict 는 **이 시장의 기사 주장이 이 시장의 데이터와 어긋날 때만** 쓴다.
   예: 기사는 반도체 매도가 원인이라는데 반도체 테마가 순매수 1위인 경우.
   아래는 어긋남이 아니다. 할 말이 있으면 points 에 써라.
     - 다른 시장 이야기 (미국 해설에 코스피 기사, 그 반대도)
     - 두 시장의 방향이 다른 것 (한쪽은 오르고 한쪽은 내린 것)
     - 기사끼리 서로 다른 이유를 대는 것
   어긋나는 게 없으면 빈 문자열로 두어라. 억지로 채우지 마라.
